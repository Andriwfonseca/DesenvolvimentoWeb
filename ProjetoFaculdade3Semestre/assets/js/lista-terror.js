...
let ajax6 = new XMLHttpRequest();

ajax6.open("GET",lista_terror); //cria a requisicao

ajax6.responseType = "json";

ajax6.send();


ajax6.onreadystatechange = async function() { //chama esse metodo quando nossa requisicao sofre alguma alteracao durante o processamento
    //quando a requisicao for processada, verificamos se ela foi finalizada
    if (ajax6.readyState == 4) { //o codigo 4 informa que ela foi finalizada

        if (ajax6.status = 200){//o codigo 200 informa que a requisicao obteve sucesso no seu processamento4

            //agora faça alguma coisa
            let resposta = ajax6.response.results;
            ...
            let conteudo_listas = document.querySelector('#list-horror');
